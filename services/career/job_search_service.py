...
class CareerJobService:
    ...
    def preferences(self) -> dict[str, str]:
        row = db.query_one("SELECT preferences_json FROM career_preferences WHERE id = 1")
        saved = db.loads(row["preferences_json"], {}) if row else {}
        return {**DEFAULT_PREFERENCES, **(saved or {})}

    def save_preferences(self, preferences: dict[str, Any]) -> dict[str, str]:
        clean = {
        ...
        db.execute(
            """
            INSERT OR REPLACE INTO career_preferences (id, preferences_json, updated_at)
            VALUES (1, ?, ?)
            """,
            (db.dumps(clean), time.time()),
        )
        return clean

    def profile(self) -> dict[str, Any]:
        row = db.query_one("SELECT cv_text, updated_at FROM career_profile WHERE id = 1")
        if not row:
            return {"cv_text": "", "updated_at": None}
        return {"cv_text": row["cv_text"], "updated_at": row["updated_at"]}

    def save_profile(self, cv_text: str) -> dict[str, Any]:
        clean = str(cv_text or "").strip()
        now = time.time()
        db.execute(
            """
            INSERT OR REPLACE INTO career_profile (id, cv_text, updated_at)
            VALUES (1, ?, ?)
            """,
            (clean, now),
        )
    ...
    def list_jobs(self) -> list[dict[str, Any]]:
        ...
        rows = db.query("SELECT * FROM career_jobs ORDER BY updated_at DESC")
        return [self._row_to_job(row) for row in rows]

    def remove_duplicates(self) -> int:
        rows = db.query("SELECT * FROM career_jobs")
        ...
        if duplicate_ids:
            db.execute_many([
                ("DELETE FROM career_jobs WHERE id = ?", (job_id,))
                for job_id in duplicate_ids
            ])
        return len(duplicate_ids)

    def get_job(self, job_id: str) -> dict[str, Any] | None:
        row = db.query_one("SELECT * FROM career_jobs WHERE id = ?", (job_id,))
    ...
    def search_jobs(self, cv_text: str = "", limit: int = 10) -> dict[str, Any]:
        ...
        self.remove_duplicates()
        existing_keys = self._existing_identity_keys()
        ...
        for candidate in raw_jobs:
            ...
            try:
                ...
                if job["status"] == "saved":
                    job = self.update_status(job["id"], "found")
                saved.append(job)
    ...
    def stream_search_jobs(self, cv_text: str = "", limit: int = 10):
        preferences = self.preferences()
        query = self._search_query(preferences, cv_text)
        raw_jobs, searched_sources, skipped_sources = self._fetch_public_jobs(
            query,
    ...
    def import_url(self, url: str, cv_text: str = "") -> dict[str, Any]:
    ...
    def save_job(
        ...
        if existing:
            if cv_text.strip() and existing.get("fit_score") is None:
                return self.score_job(existing["id"], cv_text)
            return existing
        ...
        db.execute(
            ...
            (id, title, company, location, url, description, source, status,
    ...
    def score_job(self, job_id: str, cv_text: str) -> dict[str, Any]:
    ...
    def create_score_batch(self, cv_text: str = "", job_ids: list[str] | None = None) -> dict[str, Any]:
        cv_text = str(cv_text or "").strip() or self.profile()["cv_text"]
        if not cv_text:
        ...
        if active:
            ...
            db.execute(
                ...
                (id, status, cv_text, total, completed, failed, created_at, updated_at)
                VALUES (?, 'queued', ?, 0, 0, 0, ?, ?)
                """,
                (batch_id, cv_text, now, now),
            )

        candidates = self._score_batch_candidates(job_ids)
        ...
        return self.get_score_batch(batch_id)

    def get_current_score_batch(self) -> dict[str, Any] | None:
        row = db.query_one(
            """
            SELECT * FROM career_score_batches
    ...
    def get_score_batch(self, batch_id: str) -> dict[str, Any] | None:
        row = db.query_one("SELECT * FROM career_score_batches WHERE id = ?", (batch_id,))
        return self._score_batch_payload(row) if row else None

    def cancel_score_batch(self, batch_id: str) -> dict[str, Any]:
        if not db.query_one("SELECT id FROM career_score_batches WHERE id = ?", (batch_id,)):
    ...
    def _score_worker_loop(self):
        while True:
            task = db.query_one(
                """
                SELECT task.batch_id, task.job_id, batch.cv_text
            ...
            try:
                ...
                if job.get("fit_score") is None:
                    self.score_job(task["job_id"], task["cv_text"])
                db.execute(
                    """
                    UPDATE career_score_tasks
    ...
    def _score_batch_candidates(self, job_ids: list[str] | None) -> list[dict[str, Any]]:
        rows = db.query(
            """
            SELECT id, source FROM career_jobs
            WHERE fit_score IS NULL AND status NOT IN ('applied', 'skipped')
    ...
    def _existing_identity_keys(self) -> set[str]:
        keys: set[str] = set()
        for row in db.query("SELECT title, company, location, url FROM career_jobs"):
    ...
    def _find_duplicate(self, candidate: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if not candidate_keys:
            return None
        for row in db.query("SELECT * FROM career_jobs"):
