...
class _FakeConnection:
    def __init__(self, migration_versions=None):
    ...
    def execute(self, sql, params=()):
        ...
        if normalized.startswith("SELECT version FROM app_schema_migrations"):
            return _FakeResult([{"version": version} for version in self.migration_versions])
...
def test_postgres_initializer_is_locked_versioned_and_uses_identity_columns():
    ...
    assert sum(sql.startswith("INSERT INTO app_schema_migrations") for sql in statements) == 3
